...
- [ ] Are manual scripts written as idempotent scripts (`scripts/backfill-*.sh`) with SELECT verification?

### 6. Structural Refactors & Code Search

- [ ] Search for every reference to removed columns/tables/associations
- [ ] Check background jobs, admin pages, edge functions, and cron jobs for deleted associations
...
3. **Orphaned references - Deleted column/table references in edge functions, hooks, or cron jobs cause runtime errors
