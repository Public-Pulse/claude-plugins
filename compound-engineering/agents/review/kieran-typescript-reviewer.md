...
## 11. PUBLIC PULSE PROJECT CONVENTIONS

When reviewing code in this project, additionally enforce:

### Supabase Error Handling in Mutations
- Extract error messages in `mutationFn` BEFORE throwing, using `getSupabaseFunctionError()`
- NEVER use async operations inside `onError` callbacks

### TanStack Query Patterns
- Query keys must be descriptive arrays: `['email-drafts', orgId, draftId]`
- Always use `mutateAsync` for async/await flows, `mutate` for fire-and-forget
- Cache invalidation must be targeted
- Use `maybeSingle()` instead of `.single()` for optional results

### State Management
- Server state: TanStack Query hooks (never raw Supabase calls in components)
- Global UI state: Zustand stores
- App-wide data: React Context

### Citations
- PRESERVE citation markers when SAVING drafts
- STRIP citations ONLY when SENDING (via `stripCitationMarkers()`)

### Async/Await
- Always `return await fn()`, never `return fn()`

### Imports
- Use `@/` alias (maps to `src/`), never relative paths crossing directories
