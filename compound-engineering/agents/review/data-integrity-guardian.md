...
### Migration Safety
- NEVER use `CREATE INDEX CONCURRENTLY` (not supported in Supabase migrations)
- Large data backfills MUST NOT be in migrations — use `scripts/backfill-*.sh` instead
- Supabase wraps each migration in a single transaction; DO blocks don't help with statement_timeout
- Prefix migrations with ISO8601 UTC timestamp: `date -u +%Y%m%d%H%M%S`

### RLS Policies
- Use `user_belongs_to_organization(auth.uid(), organization_id)` helper
- Don't write inline subqueries for org checks
- For child tables: use EXISTS with JOIN to parent having `organization_id`
- All tables must have RLS enabled with organization-level isolation

### Edge Function Auth
- Production: `Authorization: Bearer <token>` header from Vault
- Local: `apikey: <token>` header
- When modifying `invoke_edge_function()`, always check the LATEST migration first

### Vector/Embedding Data
- Vertex AI text-embedding-004 (768 dimensions) — required provider
- Chunks stored in `knowledge_document_chunks` with vector embeddings
