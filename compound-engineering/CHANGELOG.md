...
- **`compound-rules` skill** -- Promote institutional learnings from `docs/solutions/` into `.claude/rules/*.md` files. 4-phase pipeline: extract rule candidates, determine placement via dynamic file discovery, validate against Anthropic principles (litmus test, consumer-based placement, single authority, 200-line limit), and propose changes with user approval. Supports `--auto` and `--dry-run` flags for agent composability.
- **`/workflows:compound` Phase 4** -- After documenting a solved problem, suggests running `/compound-rules` to promote the learning to a permanent AI rule.
