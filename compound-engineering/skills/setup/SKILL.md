...
```bash
test -f supabase/config.toml && test -f tsconfig.json && echo "supabase-react" ||\
...
- **Supabase-React:** `[kieran-typescript-reviewer, supabase-reviewer, tanstack-query-reviewer, code-simplicity-reviewer, security-sentinel, performance-oracle, julik-frontend-races-reviewer]`
...
options:
  ...
  - label: "Supabase-React"
    description: "React + Supabase — adds TypeScript, Supabase, TanStack Query, and race condition reviewers"
  - label: "Python"
...
- Supabase-React → `kieran-typescript-reviewer, supabase-reviewer, tanstack-query-reviewer, julik-frontend-races-reviewer`
